...
for town in TOWNS:
    ...
    for page in range(20):
        data = []
        if page == 0:
            new_url = URL + "/" + payload[0]["pages"] + "-" + town + "?pagingSize=50"
        else:
            new_url = URL + "/" + payload[0]["pages"] + "-" + town + "?pagingOffset=" + str(page * 50)+"&pagingSize=50"
        try:
...
df.to_csv(f"{day}_{month}_{year}_sahibinden_ev.csv")
